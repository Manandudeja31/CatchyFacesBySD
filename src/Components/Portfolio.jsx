...
const Portfolio = ({ items }) => {
  const tabsName = ["Bridal", "Haldi/Mehndi", "Engagement", "Party"];
  ...
  return (
    <div
      className=" w-screen h-full bg-[#f5ebeb] pt-10 pb-10 md:mt-20"
      id="Portfolio"
    >
          ...
          {tabsName.map((element, index) => {
            return (
              <div
                ...
                className={` hover:text-[#FF9999] rounded-full p-3 cursor-pointer ${
                  currentTab === element && "bg-black hover:text-white"
                }`}
              >
                {element}
              </div>
